...
fun match(ph: &Phrase, pat: &Pattern): Option_vt(@(Dictionary, Phrase)) =
	let
		fun match_loop {n: nat} (pat: &Pattern, ph: &Phrase, dict: dictionary(n)): Option_vt(@(Dictionary, Phrase)) = (
			...
			| @list_vt_cons(pat_atom(a), ps) => (
				...
				| @list_vt_cons(x, xs) =>
					...
					in
						...
						| ~None_vt() => res where {
							val res = match_loop(ps, xs, list_vt_cons((a, list_vt_cons(x, list_vt_nil())), dict))
							prval () = fold@ph
							prval () = fold@pat
						}
			...
			| @list_vt_cons(pat_bal(b1), ps) => (
				...
				| ~list_vt_cons(x, xs) when x = b1 => (
					let
						...
						// xs is now the rest of the phrase, the tail that was not between the balanced symbols
						// b1 ... b2 xs
					in
						gfree(stk);
						case+ ph_body_opt of
			...
			// These are the "base" cases of the mult and ... patterns, successful recursive calls to the
			// match function to handle those patterns will end here
			...
			| list_vt_cons(p, _) => (
				...
				in
					...
					// If we have a pattern like : =( .. ?x . *m =) we don't have to care about the limits
					// of *m because the match clause for the balanced symbols will do a recursive call with
					// ther patterns inside it. That means the next call for match will be the pattern
					// .. ?x . *m so there are no balanced symbols anymore.
